...
class PipelineManager_v1:
            ...
            for pipeline in configured_pipelines:
                try:
                    ...
                    if pipeline_status == "STOPPED":
                        ...
                        self.updated_current_checkpoint_offsets(pipeline_config_json, config_pipeline_index)
                        ...
                        if self.is_job_hanged(response_app_jobs, config_pipeline_index):
                            pipeline_config_json = self.get_pipeline_config_json(pipeline_name)
                            pipeline_config_json = self.get_updated_pending_restart_count(pipeline_config_json)
                            self.kill_pipeline(pipeline_name)
                            self.updated_current_checkpoint_offsets(pipeline_config_json, config_pipeline_index)
        ...
        while index < len(data_lines):
            ...
            for topic in offset_data.keys():
                ...
                if len(spark_current_offset_dict.keys()) > 2 :
                    ...
                    outlier_list = []
                    if self.logger.level != logging.NOTSET:
                        outlier_list = self.stats_helper.get_outlier(offset_difference_outlier_check.values())
    ...
    def kill_pipeline_deprecated(self, pipeline_name, spark_application_id):
        is_valid = False
        # try:
        retry_count = 3
        while retry_count >= 1:
            ...
            # if retry_count == 3:
            #     self.linux_manager.process_command(self.string_manager.get_command_as_list(
            #         Constants.COMMAND_YARN_APPLICATION_KILL.format(spark_application_id)))
    ...
    # noinspection PyMethodMayBeStatic
    def get_kafka_offset_dictionary(self, topic_name, console_output):
        lines = console_output.split('\n')
        kafka_offset_dict = {}
        for line in lines:
            if topic_name not in line:
                continue
            values = line.split(':')
            partition = values[1]
            offset = int(values[2])
            kafka_offset_dict[partition] = offset
        return kafka_offset_dict

    def get_earliest_kafka_offsets(self, topic_name):
        kafka_earliest_offset_command = self.config_manager.get_config_value(
            Constants.CONFIG_COMMAND_KAFKA_EARLIEST_OFFSET).format(topic_name)
        command_list = self.string_manager.get_command_as_list(kafka_earliest_offset_command)
        self.logger.info("Earliest kafka command list - {0}".format(str(command_list)))
        output = self.linux_manager.process_command(command_list)
        self.logger.info("Earliest kafka command output - {0}".format(output))
        kafka_offset_dict = self.get_kafka_offset_dictionary(topic_name, output)
        self.logger.info("Kafka Earliest Offset dictionary {0}".format(kafka_offset_dict))
        return kafka_offset_dict

    def get_latest_kafka_offsets(self, topic_name):
        kafka_latest_offset_command = self.config_manager.get_config_value(
            Constants.CONFIG_COMMAND_KAFKA_LATEST_OFFSET).format(topic_name)
        command_list = self.string_manager.get_command_as_list(kafka_latest_offset_command)
        self.logger.info("Latest kafka command list - {0}".format(str(command_list)))
        output = self.linux_manager.process_command(command_list)
        self.logger.info("Latest kafka command output - {0}".format(output))
        kafka_offset_dict = self.get_kafka_offset_dictionary(topic_name, output)
        self.logger.info("Kafka Latest Offset dictionary {0}".format(kafka_offset_dict))
        return kafka_offset_dict
